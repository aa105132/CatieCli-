...
**方式 A：用 IP+端口 访问**（如 `http://1.2.3.4:5001`）

需要开放端口：

- 在 1Panel 左侧点击 **"主机"** → **"防火墙"**
- 点击 **"创建规则"**
- 填写：协议 `TCP`，端口 `5001`，策略 `放行`
- 点击确认
- ⚠️ 云服务器还需在控制台"安全组"放行此端口

**方式 B：用纯域名访问**（推荐，如 `https://api.example.com`）

不需要开放 5001 端口！配置反向代理后：

- 访问地址就是纯域名，不带端口号
- 比如 `https://cat.example.com`，而不是 `https://cat.example.com:5001`
- 走的是 80（HTTP）或 443（HTTPS）端口，更安全更专业
- 具体配置看下面 **第五步**
