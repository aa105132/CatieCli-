...
export default function AntigravityCredentials() {
  ...
  // 懒加载状态
  const [displayCount, setDisplayCount] = useState(5); // 默认显示5个
    ...
    <div className="min-h-screen">
      ...
      <nav className="bg-dark-900 border-b border-dark-700">
        <div className="max-w-4xl mx-auto px-3 sm:px-4 py-3 sm:py-4 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            <TaijiIcon className="w-6 h-6 sm:w-8 sm:h-8 text-purple-400" />
            <span className="hidden sm:inline text-xl font-bold">同尘</span>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <>
            <div className="space-y-3">
              {credentials.slice(0, displayCount).map((cred, index) => (
                <div
                  key={cred.id}
            ...
            {/* 加载更多按钮 */}
            {credentials.length > displayCount && (
              <div className="text-center mt-4">
                <button
                  onClick={() => setDisplayCount(prev => prev + 5)}
                  className="px-6 py-2 bg-dark-700 hover:bg-dark-600 text-gray-300 rounded-lg text-sm font-medium transition-colors"
                >
                  加载更多 ({displayCount}/{credentials.length})
                </button>
              </div>
            )}
            
            {/* 已全部加载提示 */}
            {credentials.length > 5 && credentials.length <= displayCount && (
              <div className="text-center mt-4 text-gray-500 text-sm">
                已加载全部 {credentials.length} 个凭证
              </div>
            )}
          </>
        )}
