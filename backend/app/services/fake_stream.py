"""
假流式处理模块 - 从 gcli2api 完整复制

提供将非流式响应转换为流式格式的功能，支持:
1. OpenAI 格式假流式
2. Anthropic 格式假流式
3. Gemini 格式假流式
"""
...
def _convert_usage_metadata(usage_metadata: Dict[str, Any]) -> Optional[Dict[str, int]]:
    """
...
def parse_response_for_fake_stream(response_data: Dict[str, Any]) -> Tuple[str, str, str, List[Dict[str, Any]]]:
...
def extract_fake_stream_content(response: Any) -> Tuple[str, str, Optional[Dict[str, int]]]:
    """
    ...
    try:
        ...
        # 如果完全没有内容，提供默认回复
        if not content:
            log.warning(f"No content found in response")
            content = "[响应为空，请重新尝试]"
...
def _build_candidate(parts: List[Dict[str, Any]], finish_reason: Optional[str] = "STOP") -> Dict[str, Any]:
    """构建标准候选响应结构"""
    return {
        "candidates": [{
